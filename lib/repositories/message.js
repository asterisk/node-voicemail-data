...
function createApi(table, provider, sqlGenerator, dependencies) {
  ...
  return {
    ...
    countByMailbox: function(mailbox) {
      dependencies.logger.trace('message.countMailbox called');

      return count({'mailbox': mailbox});
    },
    ...
    all: function(mailbox, folder) {
      ...
      return count({'mailbox': mailbox, 'folder': folder})
     ...
     */
    removeByMailbox: function remove(mailbox) {
        ...
        .then(function(transaction) {
          ...
          return transaction.runQuery(query)
            .then(function(result) {
              ...
              var x;
              for (x in messages) {
                messages[x] = convertFromStorage(messages[x]);
              }
  ...
  function count(fields) {
    ...
    if ('mailbox' in fields) {
      where = table['mailbox_id'].equals(fields.mailbox.getId());
    }

    if ('folder' in fields) {
      var component = table['folder_id'].equals(fields.folder.getId());
      if (where) {
        where = where.and(component);
      } else {
        where = component;
      }
    }
    ...
    if (!where) {
      dependencies.logger.error('message.count invoked without mailbox ' +
                                'or folder');
      return;
    }
