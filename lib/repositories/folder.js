...
function createApi(table, provider, sqlGenerator, dependencies) {
  ...
  return {
    ...
    all: function() {
      ...
      return find(dependencies, common, table, provider, this)
    ...
    findByNameOrDTMF: function(name, dtmf) {
      ...
      return find(dependencies, common, table, provider, this,
                  table.name.equals(name).or(table.dtmf.equals(dtmf)));
    },
...
function find(dependencies, common, table, provider, source, where) {
  dependencies.logger.trace('folder.find called');
  ...
  return common.find(query, provider, constructor.bind(source))
    .then(function(result) {
      dependencies.logger.debug({
        folders: result
      }, 'Folders loaded');
